...
pb::_get_pb_desc() {
	local name="$1"
	local repo_index=$(pbio::get_pb_repo_config)
...
pb::_install_pb() {
	local name="$1"
	local src_path="$(pbio::get_pb_path)/${name}$(pb::_get_pb_relpath $name)"
	local install_path="$(pbio::get_pb_install_path)/${name}"
	local lock="$(pbio::get_pb_path)/${name}.lock"
